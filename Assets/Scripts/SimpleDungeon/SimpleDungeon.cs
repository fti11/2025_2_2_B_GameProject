...
public class SimpleDungeon : MonoBehaviour
{
    ...
    [Header("스포너 설정")]
    public bool spawnEnemies = true;                // 일반 방과 보스 방에 적을 생성할지 여부
    public bool spawnTreasures = true;              // 보물 방에 보물을 생성할지 여부
    public int enemiesPerRoom = 2;                  // 일반 방 1개당 생성할 적의 수

    private Dictionary<Vector2Int, Room> rooms = new Dictionary<Vector2Int, Room>();        // rooms : 방 중심 좌표 -> 방 정보 매핑, 방 메타데이터 보관
    ...
    // 마지막 생성 방은 보스 방으로 지정, 일반 방 일부를 보물 방으로 변환
    void CreateRooms()
    {
        // 시작 방 : 기준점 (0,0)에 배치 
        ...
        for (int i = 0; i < roomCount; i++)
        {
            ...
            Vector2Int[] dirs =
            {
                Vector2Int.up * 6, Vector2Int.down * 6, Vector2Int.left * 6, Vector2Int.right * 6       // 기준 방에서 상/하/좌/우 일정 거리 새 방 후보
            };
            ...
            foreach (var dir in dirs)
            {
                ...
                if (AddRoom(newPos, newSize, type)) break;          // 방 영역이 기존 바닥과 겹치지 않으면 추가 성공 -> 다음 방 생성으로 진행
            }
        }

        // 일반 방 중 일정 비율을 보물 방으로 변환
        ...
        for (int i = 0; i < treasureCount && normalRooms.Count > 0; i++)        // 무작위 일반 방을 보물 방으로 바꾼다.
        {
    ...
    void CreateCorridor(Vector2Int start, Vector2Int end)
    {
        ...
        // x축 정렬 : start.x -> end.x 로 한 칸씩 이동하며 바닥 타일 추가
        ...
        // y축 정렬 : x가 같아진 뒤 start.y -> end.y로 한 칸씩 이동 
    ...
    // 바닥 주변의 8 방향을 스캔하여, 바닥이 아닌 칸을 walls로 채운다.
    void CreateWalls()
    {
        Vector2Int[] dirs =
            {
