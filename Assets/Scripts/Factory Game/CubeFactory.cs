...
public class CubeFactory : MonoBehaviour
{
    ...
    private List<WorkRequest> requestList = new List<WorkRequest>();                        // 오청서 리스트 
    ...
    void AddMaterial()
    {
        ...
        cubeComponent.Initialize(randomType);
    ...
    void ProcessRequests()
    {
        ...
        if (products[firstRequest.productType] >= firstRequest.quantity)
        {
            ...
            requestList.RemoveAt(0);                        // 리스트에서 첫 번째 제거
        }
        else
        {
    ...
    void UpdateStackVisual(GameObject[] stackArryay, Transform basePoint)
    {
        if (basePoint == null) return;

        for (int i = 0; i < stackArryay.Length; i++)
        {
            // 스택은 아래에서 위로 쌓임
            Vector3 position = basePoint.position + Vector3.up * (i * 1.1f);
            stackArryay[stackArryay.Length - 1 - i].transform.position = position;
        }
    }
    ...
    {
        // 3초마다 자동 원료 추가
        if (Time.time - lastMaterialTime > 3)
        {
            AddMaterial();
